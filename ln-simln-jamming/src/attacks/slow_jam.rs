...
use lightning::{
    ln::PaymentHash,
    routing::gossip::{NetworkGraph, NodeId},
};
    ...
    J: ChannelJammer + Send + Sync,
{
    fn setup_for_network(&self) -> Result<(), BoxError> {
        ...
        // Validate that the target's peer has channels with the attacker sender and honest sender.
        let peer_pubkey: NodeId = self.channel_to_jam.0.into();
        let attacker_sender: NodeId = self.attacker_sender.1.into();
        let honest_sender: NodeId = self.honest_sender.1.into();

        let peer_channels = &graph
            .node(&peer_pubkey)
            .ok_or(format!(
                "Target's peer {} not found in network graph",
                peer_pubkey
            ))?
            .channels;
        ...
        for channel in peer_channels {
            if let Some(channel_info) = graph.channel(*channel) {
                if (channel_info.node_one == attacker_sender
                    && channel_info.node_two == peer_pubkey)
                    || (channel_info.node_one == peer_pubkey
                        && channel_info.node_two == attacker_sender)
                {
                    attacker_channel_present = true;
                } else if (channel_info.node_one == honest_sender
                    && channel_info.node_two == peer_pubkey)
                    || (channel_info.node_one == peer_pubkey
                        && channel_info.node_two == honest_sender)
                {
                    honest_sender_present = true;
                }
            }
        }

        if !attacker_channel_present {
            return Err("Attacker sender does not have channel with target peer".into());
        }
        if !honest_sender_present {
    ...
    /// - Jam the general and congestion resources.
    ...
    async fn run_attack(
        ...
        // Note that using jammer helpers will require calculating and adding those extra costs to
        // the attack.
