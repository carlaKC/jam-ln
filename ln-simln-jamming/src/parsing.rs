...
impl NetworkType {
    ...
    /// Returns the path to the cached honest-network reputation snapshot for this attack, or None if there is no
    /// attack network to bootstrap.
    pub fn bootstrap_cache_file(&self) -> Option<PathBuf> {
        match self {
            NetworkType::Peacetime(_) => None,
            NetworkType::AttackTime(_, a) | NetworkType::BootstrapAttackTime(_, a, _) => {
                Some(a.attack_dir.join("honest_reputation_cache.csv"))
            }
        }
    }
...
pub type ReputationSnapshot = HashMap<PublicKey, HashMap<u64, ChannelSnapshot>>;

/// Comment line written at the top of a reputation cache file recording the key it was built for.
const REPUTATION_CACHE_KEY_PREFIX: &str = "# key=";

/// Writes a honest-network reputation snapshot to `path`, prefixed with a comment line recording `key` so a later
/// run can tell whether the cache still matches its inputs. The body reuses the same CSV columns the snapshot is
/// extracted into.
pub fn write_reputation_cache(
    path: &PathBuf,
    key: &str,
    snapshot: &ReputationSnapshot,
) -> Result<(), BoxError> {
    let mut file = File::create(path)?;
    writeln!(file, "{REPUTATION_CACHE_KEY_PREFIX}{key}")?;
...
/// Reads a honest-network reputation snapshot from `path`, but only if the file exists and was built for `key`.
/// Returns `Ok(None)` for a missing file or a key mismatch (a stale cache) so the caller can rebuild rather than
/// trust outdated reputation.
pub fn read_reputation_cache(
    path: &PathBuf,
    key: &str,
) -> Result<Option<ReputationSnapshot>, BoxError> {
    ...
    if header
        .strip_prefix(REPUTATION_CACHE_KEY_PREFIX)
        .map(str::trim)
        != Some(key)
    {
        return Ok(None);
    ...
    let mut snapshot = ReputationSnapshot::new();
    let mut csv_reader = csv::Reader::from_reader(reader);
    for result in csv_reader.records() {
        let record = result?;
        let pubkey = PublicKey::from_str(&record[0])?;
        let scid: u64 = record[1].parse()?;
        snapshot.entry(pubkey).or_default().insert(
            scid,
            ChannelSnapshot {
                capacity_msat: record[2].parse()?,
                outgoing_reputation: record[3].parse()?,
                incoming_revenue: record[4].parse()?,
            },
        );
    }
...
/// Computes a stable cache key for a Phase 0 reputation snapshot from everything its result depends on: the honest
/// graph, the reputation parameters, the simulation seed and the bootstrap duration. A change in any of these makes
/// a previously cached snapshot stale.
pub fn bootstrap_cache_key(
    honest_network: &[NetworkParser],
    params: &ForwardManagerParams,
    seed: u64,
    duration_secs: u32,
) -> Result<String, BoxError> {
    // Sort channels by scid so the key is independent of the ordering in the graph file.
    let mut channels: Vec<&NetworkParser> = honest_network.iter().collect();
    channels.sort_by_key(|channel| u64::from(channel.scid));

    let preimage = format!(
        "{}|{params:?}|{seed}|{duration_secs}",
...
mod tests {
    ...
    use crate::parsing::{
        bootstrap_cache_key, get_history_for_bootstrap, read_reputation_cache,
        remove_attacker_channels, write_reputation_cache,
    };
    use crate::test_utils::{get_random_keypair, setup_test_edge, test_bootstrap_forward};

    /// The cache key is stable for fixed inputs and changes when any input (graph, seed or duration) changes.
    #[test]
    fn test_bootstrap_cache_key_changes_with_inputs() {
        let (_, node_1) = get_random_keypair();
        let (_, node_2) = get_random_keypair();
        let network = vec![setup_test_edge(ShortChannelID::from(1), node_1, node_2)];
        let params = ForwardManagerParams::default();

        let key = bootstrap_cache_key(&network, &params, 7, 100).unwrap();
        // Identical inputs reproduce the key.
        assert_eq!(key, bootstrap_cache_key(&network, &params, 7, 100).unwrap());
        // A different seed, duration or graph all change it.
        assert_ne!(key, bootstrap_cache_key(&network, &params, 8, 100).unwrap());
        assert_ne!(key, bootstrap_cache_key(&network, &params, 7, 200).unwrap());
        let other = vec![setup_test_edge(ShortChannelID::from(2), node_1, node_2)];
        assert_ne!(key, bootstrap_cache_key(&other, &params, 7, 100).unwrap());
    }

    /// A written reputation cache reads back identically for the same key, and is treated as absent for a different
    /// key or a missing file.
    #[test]
    fn test_reputation_cache_round_trip() {
        ...
        let path = std::env::temp_dir().join(format!("rep_cache_{}.csv", std::process::id()));
        write_reputation_cache(&path, "key-abc", &snapshot).unwrap();

        // Matching key reads back identically; a different key is a stale cache (None).
        assert_eq!(
            read_reputation_cache(&path, "key-abc").unwrap(),
            Some(snapshot)
        );
        assert_eq!(read_reputation_cache(&path, "key-xyz").unwrap(), None);

        std::fs::remove_file(&path).unwrap();
        // A missing file is also None, not an error.
        assert_eq!(read_reputation_cache(&path, "key-abc").unwrap(), None);
    }
