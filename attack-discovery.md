...
`reputation.csv`, and `target.txt` (a single target alias — the target is a
pure routing node). `peacetime_traffic.csv` is a git-ignored intermediate
generated by `forward-builder` to rebuild `reputation.csv`; it is not
committed and the simulator does not read it. Provisioned networks: **`ln_50`**
