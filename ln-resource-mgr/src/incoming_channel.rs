...
pub(super) struct IncomingChannel {
    ...
    /// The revenue that the channel has earned our node as the incoming forwarding pair.
    pub(super) channel_revenue: RevenueAverage,
}
...
impl IncomingChannel {
    pub(super) fn new(
        ...
        let mut channel_revenue = RevenueAverage::new(params, start_ins);
        if let Some(value) = start_value {
            channel_revenue.add_value(value, start_ins)?;
        }
